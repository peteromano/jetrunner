...
module.exports = function() {
    var util = this.util,
        shell = process.argv.splice(0,2).join(' '),
        config;
    ...
    try {
        ...
        console.error('JetRunner error - bad config file.\nUsage: ' + shell + ' --config [config.json]');
        process.exit();
    }
    ...
    if(argv.server) {
        ...
        console.log('JetRunner run - no tests to run.\nUsage: ' + shell + ' --config [config.json]');
    } else {
