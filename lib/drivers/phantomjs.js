...
PhantomJsClient.prototype = util.merge(new Client, {
    ...
    runAll: function(tests, callback) {
        ...
        for(var test in tests) {
            this.add(runOne.call(this, url.format({
    ...
    runOne: function(test, callback) {
        ...
        function done(code) {
            if(queue.length && (code == OK || client.continueOnFail)) next();
            else callback(code);
        }

        console.log('JetRunner run - testing: ' + qs.parse(url.parse(test).query).test + '...');
