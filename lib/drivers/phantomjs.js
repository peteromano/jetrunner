...
PhantomJsClient.prototype = {
    ...
    run: function(jetrunner) {
        ...
        /**
         * TODO: Find out how to use the same PhantomJS instance! (https://github.com/metaskills/mocha-phantomjs/tree/v2.0.1)
         */
        function execute() {
            var testUrl = '' + this,
                phantom = spawn('phantomjs', [driver, testUrl, client.reporter], { stdio: client.stdio || 'inherit' });

            console.log('JetRunner run - testing: ' + qs.parse(url.parse(testUrl).query).test + '...');
            console.log(('URL: ' + decodeURIComponent(testUrl)).grey);

            phantom.on('exit', function(code) {
                jetrunner.emit('phantomjs:exit');
                console.log('JetRunner run - finished: ' + (code && (IndicatorsEnum.FAILURE + ' FAIL!').red || (IndicatorsEnum.SUCCESS + ' OK').green));
                done(code);
            });
