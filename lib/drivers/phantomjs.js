...
PhantomJsClient.prototype = util.merge(new Client, {
    ...
    runOne: function(test, callback) {
        ...
        console.log('JetRunner client (' + name + ') - started' + (pid = ' (PID:' + phantom.pid + ')'));
        ...
        phantom.on('exit', function(code) {
            ...
            console.log('JetRunner client (' + name + ') - finished ' + suite +': ' + (code && (IndicatorsEnum.FAILURE + ' FAIL!').red || (IndicatorsEnum.SUCCESS + ' OK').green));
            console.log('JetRunner client (' + name + ') - shut down' + pid);
            done(code);
        });
