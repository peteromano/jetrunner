...
PhantomJsClient.prototype = util.merge(new Client, {
    ...
    runAll: function(tests, callback) {
        ...
        for(var test in tests) {
            this.addRunner(runOne.call(this, this.buildTestURL(tests, test), callback));
        }
    ...
    runOne: function(test, callback) {
        ...
        console.log('JetRunner client - PhantomJS - started' + (pid = ' (PID:' + phantom.pid + ')'));
        console.log('JetRunner client - PhantomJS - testing ' + suite + '...');
        console.log(('URL: ' + decodeURIComponent(test)).grey);

        phantom.on('exit', function(code) {
            JetRunner.emit('phantomjs:exit');
            console.log('JetRunner client - PhantomJS - finished ' + suite +': ' + (code && (IndicatorsEnum.FAILURE + ' FAIL!').red || (IndicatorsEnum.SUCCESS + ' OK').green));
            console.log('JetRunner client - PhantomJS - shut down' + pid);
            done(code);
        });
