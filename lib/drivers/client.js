...
Client.prototype = util.createEventEmitter({
    ...
    initialize: function(tests, config) {
        ...
        this.config = config || {};
        return this.on('complete', function(err) {
...
Client.getTestPath = function(tests, test) {
    return typeof tests[test] !== 'string' ? Object.keys(typeof test === 'number' ? tests[test] : test)[0] : tests[test];
};
...
Client.getSrcPath = function(tests, test) {
    return typeof tests[test] !== 'string' && (test = typeof test === 'number' ? tests[test] : test)[Object.keys(test)[0]] || '';
};
