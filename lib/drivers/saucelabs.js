...
SauceLabsClient.prototype = util.merge(new TunnelClient, {
            ...
            self = this.once('connect', function() {
                ...
                function complete() {
                    self._reporter.end(function(err) {
                        self.emit(!err && result == TestStatusEnum.PASS ? 'success' : 'failure', result || undefined);
                        self.emit('complete', err || result);
                    });
                }
                    ...
                    this.setContext('sauce:job-info={"passed": ' + !failed + '}', function() {
                        browser.testComplete(function() {
                            ...
                            if(err) {
                                ...
                                self._reporter.addSuite(data.payload.suite, job);
                            }

                            if(config.debug) {
                                Logger.log(('\njetstream ' + jetstream).grey);
                            }
    ...
    _spawn: function() {
        var client = this.config.client,
            ...
            args = ['-jar', tunnel.path, client.username, client.key, '-F', tunnel.fastFailRegexps, '-l', tunnel.logFile, '-f', tunnel.logFile/*, '-i', TUNNEL_ID*/];
...
JetStream.CONNECTION_ERROR = {
    ...
    payload: null,
    errors: [
        'JetStream is invalid. Make sure your test runner is set up correctly.',
...
function Reporter(config) {
    this.config = config || {};
    this._stream = new tap.Producer(this.config.debug)
                    .on('data', this._read.bind(this));
}
...
Reporter.prototype = {
    ...
    addSuite: function(suite, job) {
        new Suite(suite, job).forEach(this.addResults.bind(this));
        return this;
    },

    addResults: function(results) {
        this._stream.write(results);
        return this;
    },
    ...
    _write: function(callback) {
        ...
        function write(err, count, ok) {

            if(!err) {
                try {
                    fs.writeFile(reportFile, buffer, callback);
                } catch(e) {
                    callback(failOnError && err || e);
                }

                Logger.log('\n' + buffer);
            } else {
                callback(failOnError && err || null);
            }
        }

        callback = callback || function() {};
        this._stream.once('end', write).end();
        return this;
    }
 ...
 * @constructor
 */
function Suite(suite, job) {
    this._process(suite, job && [job + ':']);
}

Suite.prototype = util.merge([], {

    _process: function(suite, desc) {
        suite   = typeof suite === 'object' && suite || {};
        desc    = desc || [];

        var tests = suite.tests, test;

        if(tests) {
            while(test = tests.shift()) {
                this.push(this._formatResult(test, desc.concat(test.title).join(' ')));
            }
        }

        for(var next in suite) {
            if(suite.hasOwnProperty(next)) {
                this._process(suite[next], desc.concat(next));
            }
        }
    },

    _formatResult: function(test, name) {
        var TestStatusEnum = Client.TestStatusEnum,
            passed = Number(test.state === TestStatusEnum.PASSED),
            result = new Results({
                ok:         !!passed,
                skip:       Number(test.pending),
                skipPass:   0,
                skipFail:   0,
                todo:       0,
                todoPass:   0,
